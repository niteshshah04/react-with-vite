  ...
  const cleanData = useCallback((data: IBullishOIResponse) => {
    return Object.keys(data).map((key, index) => {
      ...
      return {
        ...
        CE_SB: lastRecord.CE_ShortBuildup?.toFixed(2) ? lastRecord.CE_ShortBuildup?.toFixed(2) : 0,
        CE_LB: lastRecord.CE_LongBuildup?.toFixed(2) ? lastRecord.CE_LongBuildup?.toFixed(2) : 0,
        PE_LB: lastRecord.PE_LongBuildUp?.toFixed(2) ? lastRecord.PE_LongBuildUp?.toFixed(2) : 0,
        PE_LU: lastRecord.PE_LongUnwinding?.toFixed(2 ) ? lastRecord.PE_LongUnwinding.toFixed(2) : 0,
        PE_SB: lastRecord.PE_ShortBuildUp?.toFixed(2) ? lastRecord.PE_ShortBuildUp.toFixed(2) : 0,
        CE_SC: lastRecord.CE_ShortCovering?.toFixed(2) ? lastRecord.CE_ShortCovering.toFixed(2) : 0,
        PE_SC: lastRecord.PE_ShortCovering?.toFixed(2) ? lastRecord.PE_ShortCovering.toFixed(2) : 0,
